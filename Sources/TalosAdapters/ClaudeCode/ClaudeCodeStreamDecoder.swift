...
struct ClaudeCodeStreamDecoder {
    ...
    /// The held call's string arguments kept keyed, so a board write's item
    /// and target column can be read by field name later. Which key means
    /// what is the recognizer's knowledge, not this decoder's.
    private static func keyedArguments(from input: [String: Any]) -> [String: String] {
        input.compactMapValues { $0 as? String }
    }
}
