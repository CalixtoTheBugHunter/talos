...
so directly: `lint` "is SwiftLint plus SwiftFormat today, so this needs custom rules somebody writes
— or it is review-only and the rule is an honor system", blocking "The a11y and lint CI stages".
Until that is decided, treat a hex literal or a fixed point size as **review-enforced** and do not
claim `lint` catches it.
