...
**The handed guidance is the human's own run, through the UI** — their own build of the app (Product →
Run), its controls, and Project Library files they author. No terminal command, no bundle the reviewer
built to a path, and no state the reviewer fabricated (anything derived that only Talos writes, such as
a database under `.talos/local/`): a user cannot produce it, so exercising it verifies plumbing rather
than the experience the gate is for. Where the PR has no user-visible surface, step 1's regression check
*is* the verification. Where a criterion has no UI path yet, that is a finding naming the missing entry
point, not a fixture to build by hand.
