...
| [`review-epic`](review-epic/SKILL.md) | ✅ | Closing an item that is an **epic** — `create-issue`'s delegate for that case, never entered directly | None on its own — `create-issue` still owns `In review` → `Done`; this skill supplies the epic-specific content | The [fast path](https://github.com/CalixtoTheBugHunter/talos/wiki/Engineering-Standards#closing-an-epic-the-review-epic-fast-path) — every sub-issue actually `Done`, the epic's own done condition against its Spec Pages, and a smoke test for regression, never the leaves' criteria restated |
...
**All five transition-owning workflow skills are in the repo, plus `review-epic`.** `spec-driven-change`
still runs first on every change — it covers any Talos change and is the default entry point — and the
other four own the transitions listed above. `review-epic` owns no transition of its own; per
[decision 65](https://github.com/CalixtoTheBugHunter/talos/wiki/Decision-Log), it is `create-issue`'s
delegate for the one input shape — an epic — where closing needs different content than a leaf. A
change that matches none of the four transition-owning skills is not unguarded; it is guarded by
