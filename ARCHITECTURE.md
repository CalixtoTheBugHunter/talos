...
| `TalosSafeguards` | `TalosCore`, `TalosProjectLibrary`, `TalosAdapters` | The tiered, deny-by-default [Safeguards gate](https://github.com/CalixtoTheBugHunter/talos/wiki/Safeguards-and-Autonomy). Reads the guideline, Safeguards, and connectors documents from `TalosProjectLibrary` for its pre-check, and decides a permission request from `TalosAdapters`. Depends on neither the orchestration layer nor the UI, so the gate stays below the pipeline that calls it. |
