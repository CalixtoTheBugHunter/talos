...
and the structural banner-adjacency ban
[decision 77](https://github.com/CalixtoTheBugHunter/talos/wiki/Decision-Log#engineering-decisions) adds.
...
- [Decision Log § Engineering decisions](https://github.com/CalixtoTheBugHunter/talos/wiki/Decision-Log#engineering-decisions) — decisions 70, 76, and 77
...
| 4 | A plain `//` comment block directly adjacent to a `///` doc comment (at most one blank line between) | DocC never renders a `//` block, so one sitting right beside a `///` block only duplicates it or belongs folded into it — a `// MARK:`-only block is exempt |
...
| **Whether a `//` block's *content* actually duplicates the `///` block beside it** | Check 4 is adjacency only. It does not read either block's words — a `//` banner right next to a `///` doc comment is folded in or deleted regardless of whether the two say the same thing, because DocC will never render the `//` one either way. |
