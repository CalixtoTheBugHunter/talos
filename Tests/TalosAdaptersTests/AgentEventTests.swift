...
struct AgentEventTests {
    ...
    /// intercepts.
    @Test("A tool call arrives as a tool call, naming the tool and its target")
    func aToolCallArrivesAsAToolCall() async throws {
        let events = try await Self.run { adapter in
            await adapter.emit(.toolCall(AgentToolCall(id: "call-1", name: "write_file", target: "README.md")))
        }
    ...
    func aToolCallAndAPermissionRequestForTheSameToolAreNotEqual() {
        let call = AgentEvent.toolCall(AgentToolCall(id: "x", name: "write_file", target: "README.md"))
